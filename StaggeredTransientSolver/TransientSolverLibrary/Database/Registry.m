function varargout = Registry(Intent,varargin)
    
    % ============================================================================ %
    %                    Persistence and Initialization                            %
    % ============================================================================ %
    % Declare TheRegistry as persistent.  This makes permanent storage for
    % the variable and is only cleared when the base workspace is flushed
    % or a function is modified.
    persistent TheRegistry Options
    
    
    % If TheRegistry is newly created, it will be an empty array and will be
    % initialized as an empty struct.
    if isempty(TheRegistry)
        TheRegistry = struct();
        
        Options = struct();
        Options.InterstionsCannotOverwriteExistentFields = true;
    end
    
    
    
    % ============================================================================ %
    %                    Registry-Wide Information/Diagnostics                     %
    % ============================================================================ %
    ...
    if (nargin == 1) && IsNotEmpty(Intent) && strcmp('_',Intent(1))
        switch(strtrim(lower(Intent)))
            case ('_fieldnames')
                varargout{1} = fieldnames(TheRegistry);
            case ({'_all','_registry'})
                varargout{1} = TheRegistry;
            case ({'_whos','_information'})
                varargout{1} = whos('TheRegistry');
        end
    ...
    %                              Input Error checks                              %
    % ============================================================================ %
    ThrowingID = 'TransientSolver:Registry:';
    ...
    % Semantic renaming of the data passed to the Registry().
    IntentData = varargin;
    
    % Boolean Mask indicating which strings in IntentData are fields in the registry
    Mask       = isfield(TheRegistry,IntentData);
    ...
    if any(Mask)
        ...
        switch(lower(strtrim(Intent)))
            ...
            case {'retrieve','get'}
                GetRequestedDataFromRegistry();
                
            case {'assign','set'}
                AssignPassedDataToRegistry();
                varargout{1} = true;
                
            case 'delete'
                DeleteRequestedDataFromRegistry();
                varargout{1} = true;
        ...
        switch(strtrim(lower(Intent)))
            case 'initialize'
                InitializeRegistryWithPassedData();
                varargout{1} = true;
                
            case 'insert'
                InsertPassedDataIntoRegistry();
                varargout{1} = true;
            otherwise
    ...
    % ----------------------- Main Routine ----------------------- %
    function [] = GetRequestedDataFromRegistry()
    ...
    %                               Assignment                                     %
    % ============================================================================ %
    function [] = AssignPassedDataToRegistry()
        ...
        for k = 1:length(FieldNames)
            ...
            EntrySize = size(TheRegistry.(FieldName));
            ...
            if isempty(AssignmentIndices)
                ...
                TheRegistry.(FieldName) = AssignmentValue;
                
            else
                % Without subscripts given.
                TheRegistry.(FieldName)(AssignmentIndices) = AssignmentValue;
    ...
    %                              Insertion                                       %
    % ============================================================================ %
    function [] = InsertPassedDataIntoRegistry()
        
        FieldNames = IntentData(1:2:end);
        FieldData  = IntentData(2:2:end);
        Ninsert    = length(FieldNames);
        
        if isfield(Registry,FieldNames)
            if Options.InterstionsCannotOverwriteExistentFields
        ...
        for k = 1 : Ninsert
            TheRegistry.(FieldNames{k}) = FieldData{k};
        end
    ...
    %                                  Deletion                                    %
    % ============================================================================ %
    function [] = DeleteRequestedDataFromRegistry()
        TheRegistry = rmfield(TheRegistry,FieldNames);
    end
    ...
    function Indices = GetLinearIndices(DataSource,EntrySize_)
        ...
        if IsNotEmpty(Subscripts)
            ...
            try
                ...
                error('TransientSolver:Registry:ImproperArraySubscripts'    ,...
