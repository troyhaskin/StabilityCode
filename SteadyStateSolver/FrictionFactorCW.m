function [fSol,varargout] = FrictionFactorCW(Reynolds,EpsRelative,fGuess)
    ...
    Rez   = @(f,Re) CWResidual(EpsRelative,Re,f);
    DRez  = @(f,Re) CWResidualFirstDerivative(EpsRelative,Re,f);
    
    if (nargin < 2) || isempty(fGuess)
        f = 1E-3 + 0 * Reynolds      ;
    else
        f = fGuess;
    end
...
% ====================================================================== %
    while NotConverged
        RezIter  = @(f) Rez(f,Reynolds(Mask));
        DRezIter = @(f) DRez(f,Reynolds(Mask));
